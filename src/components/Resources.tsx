...
const Resources: React.FC<ResourcesProps> = ({ resources }) => {
  const resourceItems = [
    {
      icon: <Coins className="w-5 h-5 text-royal-gold" />,
      name: "Gold",
      value: resources.gold,
    },
    {
      icon: <Map className="w-5 h-5 text-wood-dark" />,
      name: "Land",
      value: resources.land,
    },
    {
    ...
    {
      icon: <Wheat className="w-5 h-5 text-royal-gold" />,
      name: "Food",
      value: resources.food,
    },
    {
      icon: <Trees className="w-5 h-5 text-wood-dark" />,
      name: "Wood",
      value: resources.wood,
    },
    {
      icon: <Pickaxe className="w-5 h-5 text-stone" />,
      name: "Stone",
    ...
    <div className="glass-panel p-6 mb-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
        {resourceItems.map((item) => (
          <div
            key={item.name}
            className="glass-panel p-2 rounded-sm flex items-center"
          >
